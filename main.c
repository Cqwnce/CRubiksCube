...
void printCube(char cube[6][3][3]) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < 3; j++) printf("%c ", cube[2][i][j]); // L face
        printf("  ");  // Spacing between faces

        for (int j = 0; j < 3; j++) printf("%c ", cube[0][i][j]); // F face
        printf("  ");  // Spacing between faces

        for (int j = 0; j < 3; j++) printf("%c ", cube[3][i][j]); // R face
        printf("  ");  // Spacing between faces
        
        for (int j = 0; j < 3; j++) printf("%c ", cube[1][i][j]); // B face
        printf("\n");
    };
